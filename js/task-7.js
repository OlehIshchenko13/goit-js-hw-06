// Напиши скрипт, который реагирует на изменение значения input#font-size-control (событие input) 
// и изменяет инлайн-стиль span#text обновляя свойство font-size. В результате при перетаскивании ползунка будет меняться размер текста.
const rangeRef = document.querySelector(
    'input#font-size-control[type="range"]',
  );
  
  const sizedRef = document.querySelector('span#text');
  
  rangeRef.addEventListener('input', elem => {
    sizedRef.style.fontSize = `${rangeRef.value}px`;
    
  });
